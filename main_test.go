...
func setupGit(t *testing.T, proj string) func() {
	...
	tempDir, err := ioutil.TempDir("", "ci-gotest")
	if err != nil {
		t.Fatal(err)
	}
	...
	var gitCmdList = []struct {
		...
		{
			[]string{"commit", "-m", "ci-go test"},
			projPath,
			[]string{
				"GIT_COMMITTER_NAME=ci-go",
				"GIT_COMMITTER_EMAIL=test@example.com",
				"GIT_AUTHOR_NAME=ci-go",
...
func TestHelperProcess(t *testing.T) {
	...
	if os.Args[2] == "git" {
		fmt.Fprintln(os.Stdout, "everything updated")
		os.Exit(1)
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case err := <-errCh:
				...
				case rec := <-expSigCh:
					if rec != tc.sig {
						t.Errorf("expected signale %q got %q", tc.sig, rec)
					}
				default:
